	...
	//***************json获取*******************************
	  ...
	  public static  String readAll(Reader rd) throws IOException {  
		...
		public static String getFileDir(DownLoadInfo downInfo){
			if(downInfo.getDownPath()==null){
				//临时创建  天龙八部001乐视235673
		  		StringBuffer local=new StringBuffer(FileUtils.getFileCaches());
		  		local.append("/");
		  		local.append(downInfo.getDownName());
		  		local.append(String.format("%03d", downInfo.getDownPosition()));
		  		local.append(downInfo.getDownTag());
		  		local.append(downInfo.getDownID());
		  		File file =new File(local.toString());
		  		if(!file.exists()||!file.isDirectory()){
		  			file.mkdir();
		  		}
		  		downInfo.setDownPath(file.getAbsolutePath());//文件夹存放路径
		  		DBHelperDao.getDBHelperDaoInstace().updataMovieStoreLocal(downInfo.getDownUrl(), file.getAbsolutePath());
			}
			return downInfo.getDownPath();
	      }
	  	public static String urlToFileName(DownLoadInfo downInfo,String realUrl)
	      {
	  		...
	  		local.append(String.format("%03d", downInfo.getDownPosition()));
	  		local.append("_");
	  		local.append(String.format("%02d", downInfo.getDownIndex()));
