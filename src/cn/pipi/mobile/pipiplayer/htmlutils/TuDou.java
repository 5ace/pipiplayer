...
public  class TuDou implements HtmlInterface{
	
	  private  final String TAG = "TAG999 tudou";
	 //大于2-->hd2  大于1->mp4  其他 -->flv
	 
	  public  ArrayList<String> getDownloadInfo(String url,int parseMode) {
			 String vcode=HtmlUtils.getHtml(url,"vcode:");
			 if(!TextUtils.isEmpty(vcode)){
				 try {
					 return parseVideoItem(vcode, parseMode);
				} catch (Exception e) {
	        ...
	        try {
	           // JSONObject jsonObject = JsonReader.readJsonFromUrl(url);
	            ...
	            for(int i=0;i<vArray.length();i++){
	                ...
	                if (BuildConfig.DEBUG) Log.i(TAG, "url= "+su);
	                downloadInfo.add(su);
	            }
	        } catch (Exception e) {
