...
public class Thumbnailer implements Runnable {
    ...
    public void run() {
        ...
        while (!isStopping) {
            ...
            while (mItems.size() == 0) {
                try {
                    MainLocalActivity.hideProgressBar(mContext);
                    MainLocalActivity.clearTextInfo(mContext);
                    totalCount = 0;
                    notEmpty.await();
            ...
            MainLocalActivity.showProgressBar(mContext);

            MainLocalActivity.sendTextInfo(mContext, String.format("%s %s", mPrefix, item.getFileName()), count, total);
            count++;
